...
{
    public class ModbusManage: IDriver
    {
        IPLC  PLC;
        ...
        #region 构造方法
        public ModbusManage(string IP,bool IsAuto=true)
        {
            int port = 502;
            var ss = IP.Split(':');
            if (ss.Length > 1)
            ...
            PLC = ModbusPLC.Create();
            
            if (IsAuto == false)
            {//短连接
                PLC.BasicClient.ConnectAsync(ss[0], port, 5000).GetAwaiter().GetResult();
                return;
            }
            IsAutoConnet = IsAuto;
            ...
            //    try
            //    {
            //        await PLC.BasicClient.ConnectAsync(ss[0], port, 3000);
            //    }
            //    catch (Exception e)
            //    {
        ...
        public T ReadAddress<T>(string Address,int len=1)
        {
            ...
            if (typeof(T) == typeof(bool))
            {
                ...
                if (bit > -1 || ((ModbusDevice)dv).memoryType== MemoryType.Bit)
                {//直接返回位值
        ...
        private bool CheckAddress(string Address, out string type, out int num,out int bit)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception(Address+"地址格式错误。格式=功能码#起始地址");
                }
                type = ss[0];
                if (ss[1].Contains("."))
                {
                    var sn = ss[1].Split('.');
                    num = Convert.ToInt32(sn[0]);
                    bit = Convert.ToInt32(sn[1]);
                }
                else
                {
