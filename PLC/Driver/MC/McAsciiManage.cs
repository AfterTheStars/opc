...
namespace PLC.MC
{
    public class McAsciiManage: IDriver
    {
        public IPLC  PLC { get; set; }
        public Dictionary<string, BaseDevice> DicDevice { get; set; } = new Dictionary<string, BaseDevice>();//PLC地址
        ...
        #region 构造方法
        public McAsciiManage(string IP,bool IsAuto=true,bool IsUDP=false,bool IsByte=false)
        {
            int port = 5000;
            var ss = IP.Split(':');
            if (ss.Length > 1)
            ...
            if (IsByte)
            {
                PLC = McBytePLC.Create(IsUDP);
            }
            else
            {
                PLC = McAsciiPLC.Create(IsUDP);
            }
        ...
        public T ReadAddress<T>(string Address,int len=1)
        {
            ...
            if (typeof(T) == typeof(bool))
            {
                ...
                if (bit > -1 || ((McDevice)dv).memoryType== MemoryType.Bit)
                {//直接返回位值
